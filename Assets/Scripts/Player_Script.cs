...
public class Player_Script : MonoBehaviour 
{
	...
	public float fuelConsumitionInterval = 2.5f;	//Fuel consumition interval
	...
	void FixedUpdate ()
	{
		...
		_rigidbody2D.velocity = movement * speed; 							//Add Velocity to the player ship rigidbody

		//Lock the position in the screen by putting a boundaries
		_rigidbody2D.position = new Vector2 
			(
				Mathf.Clamp (_rigidbody2D.position.x, boundary.xMin, boundary.xMax),  //X
				Mathf.Clamp (_rigidbody2D.position.y, boundary.yMin, boundary.yMax)	 //Y
			);
	}
	...
	{
		while (SharedValues_Script.fuel > 0) 
		{
			SharedValues_Script.fuel--;

			yield return new WaitForSeconds (fuelConsumitionInterval);
		}
