...
public class Asteroid_Script : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		_rigidbody2D.angularVelocity = Random.Range(minTumble, maxTumble); 		//Angular movement based on random speed values
		_rigidbody2D.velocity = -1 * transform.up * speed; 						//Negative Velocity to move down towards the player ship
	}
